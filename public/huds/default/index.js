...
function updateRoundState(phase, round, map, previously, bomb, players) {
  ...
  switch (phase.phase) {
    ...
    case "defuse":
      ...
      break;
    case "paused":
      updateStatePaused(phase, "paused");
      break;
    case "timeout_t":
      updateStatePaused(phase, "timeout_t");
      break;
    case "timeout_ct":
      updateStatePaused(phase, "timeout_ct");
      break;
  }
}
...
function updateStateFreezetime(phase, previously) {
  if (phase) {
    ...
    } else if (checkPrev(previously, "paused") || checkPrev(previously, "timeout_t") || checkPrev(previously, "timeout_ct")) {
...
function updateStatePaused(phase, type) {
  resetBomb();
  $("#players_left #box_utility").slideDown(500);
  $("#players_right #box_utility").slideDown(500);
  $("#alert_middle").removeClass();
  if (type == "paused") {
    animateRoundTimer("pause_active", true);
