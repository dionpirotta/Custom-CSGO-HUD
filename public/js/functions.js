...
function loadTeam(team) {
    ...
    $("#logo_img").attr("src", (team && team.logo ? "/storage/" + team.logo : "")).hide();
    ...
    loadHUDs(function (res) {
        ...
        huds.forEach(function (hud) {
            ...
            if (!files[hud].includes("template.pug") || !files[hud].includes("index.js") || !files[hud].includes("style.css")) {
                ...
                $status.addClass("tooltipped").attr({
                    "data-position": "top",
                    "data-tooltip": tip.substr(0, tip.length - 1)
                }).text("warning")
            } else {
...
function loadPlayer(player) {
    ...
    $("#id").val(player ? player._id : "");
    $("#avatar_img").attr("src", (player && player.avatar ? "/storage/" + player.avatar : "")).hide();
    if (player && player.avatar) $("#avatar_img").show();
    $("#country").formSelect();
